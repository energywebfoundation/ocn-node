...
interpret that as the deletion of all rules for the given list. Therefore, the specified list will be deactivated. It is also possible to set which specific module will be blocked/blacklisted for the specific sender. If an empty list of modules is sent then the sender is eligible to use all the modules. If
