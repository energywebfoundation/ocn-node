...
interpret that as the deletion of all rules for the given list. Therefore, the specified list will be deactivated. 
It is also mandatory to set which modules will be blocked/blacklisted for the sender. If an empty list of modules 
is sent then the sender is eligible to use all the modules. If updating `signatures` or setting `block-all`, then 
no body is required.
