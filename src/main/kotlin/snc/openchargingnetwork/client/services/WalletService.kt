...
class WalletService(walletRepo: WalletRepository,
    ...
    fun verify(request: String, signature: String, sender: BasicRole) {
        ...
        val registeredClientAddress = registry.clientAddressOf(sender.country.toByteArray(), sender.id.toByteArray()).sendAsync().get()
        if (signingAddress.toLowerCase() != registeredClientAddress.toLowerCase()) {
